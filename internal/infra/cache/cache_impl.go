...
func (c *CacheImpl) Set(ctx context.Context, key string, data interface{}, options Options) (bool, error) {
	...
	if err != nil {
		...
		logger.Log.Error().Msgf("Failed for marshaling data:", err)
		return false, err
	}
	...
	if options.Compress {
		...
		if err != nil {
			logger.Log.Error().Msgf("Failed for compress data:", err)
			return false, err
		}
	}
...
func (c *CacheImpl) Get(ctx context.Context, key string, data interface{}) (interface{}, error) {
	...
	if err != nil {
		logger.Log.Error().Msgf("Failed for unMarshaling data:", err)
		return nil, err
	}
...
func (c *CacheImpl) Delete(ctx context.Context, key string) (bool, error) {
	...
	if err != nil {
		logger.Log.Error().Msgf("Failed for delete data on redis:", err)
		return false, err
	}
